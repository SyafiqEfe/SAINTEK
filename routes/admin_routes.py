...
# from models.user import User # Hapus impor ini
...
# Admin required decorator (diubah agar tidak bergantung pada flask_login)
def admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Untuk sementara, kita akan mengizinkan akses penuh ke admin panel
        # Anda perlu mengimplementasikan mekanisme otentikasi admin Anda sendiri di sini
        # Misalnya, memeriksa variabel lingkungan atau token sederhana
        # if not is_admin_authenticated(): # Contoh fungsi kustom
...
def index():
    ...
    # total_users = User.query.count() # Hapus baris ini
    ...
    return render_template(
        ...
        # total_users=total_users, # Hapus baris ini
...
def create_lecture():
    ...
    if form.validate_on_submit():
        lecture = Lecture(
            ...
            # user_id=current_user.id # Hapus baris ini
        )
